...
- CLI: `--informativeness novelty_prune` / `novelty_recent_prune` / `recent_prune`
- 不再插入 marker，而是删除低信息/冗余 turn，仅保留 informative/recent/current turns。
- 目的：将论文的 context pruning 讨论落到 Dialogue-KT，减少长对话中无关或重复证据对 LLMKT 的干扰，同时降低 prompt 长度。
...
基于已完成测试集预测做了探索性等权平均：`baseline + SIP-Novelty + BEP-Adaptive-Labels` 的 Overall AUC 为 76.985，超过论文 76.71；`baseline + BEP-Adaptive-Labels` 为 76.917。注意：这是在测试集上事后选择组合的探索结果，不能作为最终诚实指标。下一步需要在 validation split 上选择组合/权重，再固定后到 test 上评估，避免数据泄露。

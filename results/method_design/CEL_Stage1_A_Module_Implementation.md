...
A 模块不使用 B，也不在该模块目标中引入 `h_r/h_m` 一致性损失。它只回答：
...
A+B 模块不再另建一套选择变量，而是直接将 `a` 线性映射为互补权重：
...
- `a ⊙ H` 用于 A 模块已验证的加性增强。
- `h_r/h_n` 用于 A+B 模块的显式证据/非证据双视图训练。
...
- `dialogue_kt/main.py`
  - 相关 CLI 参数。
- `scripts/cel/`
  - A/A+B 统一训练、同步与 audit 入口。

## 10. 全局模块设计规范（适用于 A、B、C、D……）

本规范适用于 A、B 以及未来的 C、D 等所有模块。设计应首先服从研究假设和目标因果机制，而不是最小改动、最少参数或单次训练成本。每个模块都必须明确输入/输出表征、信息流、下游使用、训练目标、优化日程和诊断指标；容量要足以表达目标变换。轻量模块只能用于接口检查、sanity check 或消融，不能代表核心方法或替核心方法节省正式训练成本。

对 A 而言，这意味着先满足 task-conditioned evidence selection 的因果路径、表示接口和可训练性，再讨论参数效率；低容量 A 的失败不能直接否定 A 模块方向。

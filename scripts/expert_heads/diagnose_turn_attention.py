...
"""
import os
os.environ["SDPA_ATTENTION_BACKEND"] = "eager"  # MUST be before any torch/transformers import
...
def main():
    ...
    if concentrations:
        print(f"Concentration stats: mean={np.mean(concentrations):.3f}, "
              f"max={np.max(concentrations):.3f}, min={np.min(concentrations):.3f}")
    else:
        print("Concentration stats: NO VALID HEADS FOUND")
        verdict = "FAIL"

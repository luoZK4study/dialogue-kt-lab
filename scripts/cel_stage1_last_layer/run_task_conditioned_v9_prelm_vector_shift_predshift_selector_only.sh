...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -m dialogue_kt.main train \
  ...
  --cel_selector_init_model_name cel_task_conditioned_lastlayer_v2_selector_only_qwen3_1.7b \
  --model_name cel_task_conditioned_lastlayer_v9_prelm_vector_shift_predshift_selector_only_qwen3_1.7b \
  ...
  --cel_hook_site pre_lm_head \
  --cel_layer_idx -1 \
  --cel_selector_hidden_dim 384 \
  --cel_gamma 0.05 \
  --cel_use_norm 0 \
  --cel_application_mode prediction_token_shift \

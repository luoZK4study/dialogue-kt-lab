...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/cel_stage1_last_layer/finalize_current_formal_candidate_via_ssh_alias.sh <ssh_alias>

Purpose:
  Run the current-state strict formal finalize wrapper through a local SSH
...
Examples:
  ...
  TASK_CONDITIONED_FINALIZE_SKIP_REFRESH=1 bash scripts/cel_stage1_last_layer/finalize_current_formal_candidate_via_ssh_alias.sh --default-3090
...
echo "== Strict Formal Alias Finalize =="
...
TASK_CONDITIONED_SSH_TARGET="$SSH_ALIAS" \
TASK_CONDITIONED_SSH_OPTS= \
  bash "$ROOT_DIR/scripts/cel_stage1_last_layer/finalize_current_formal_candidate.sh"

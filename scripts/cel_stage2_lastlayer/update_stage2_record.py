...
def build_record(rows: list[dict], baseline_auc: float | None, stage: str | None, followup_event: str | None) -> str:
    ...
    if not rows:
        ...
        if baseline_auc is None:
            lines.append("- 2026-06-20：baseline recert 仍在运行，等待首个 stage2 指标同步。")
        else:
            lines.append(
                f"- 2026-06-20：baseline recert 已完成（Overall AUC **{baseline_auc:.2f}**），primary loop 正在继续推进首个 stage2 指标。"
            )
        return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    parser.add_argument("--record", default="results/cel_stage2_lastlayer/CEL_Stage2_LastLayer_DialogueKT_实验记录.md")
    parser.add_argument("--metrics_dir", default="results/cel_stage2_lastlayer/metrics")
    parser.add_argument("--baseline_metrics", default="results/baseline_recert/metrics/metrics_lmkt_qwen3_1.7b_recert_20260620.txt")
    parser.add_argument("--status", default="results/cel_stage2_lastlayer/STATUS.md")

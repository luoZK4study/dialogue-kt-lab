...
def compute_attention_weighted_aggregation(
    ...
    Args:
        kc_probs: (batch, max_kcs) probability per KC
        attention_weights: (batch, max_kcs) attention-based weight per KC
        num_kcs: (batch,) number of actual KCs per sample
        last_idxs: (batch, max_kcs) indices for masking
        agg: Aggregation method ("mean-ar", "prod", "mean-geo")

    Returns:
        corr_probs: (batch,) aggregated correctness probability
    """
    # Mask for valid KCs
    ...
    if agg == "mean-ar":
        ...
        corr_probs = weighted_sum / weight_sum
    elif agg == "prod":
        # Weighted product in log space
        log_probs = torch.log(kc_probs.clamp(min=1e-8))
        weighted_log_sum = (log_probs * attention_weights * mask).sum(dim=1)
        weight_sum = (attention_weights * mask).sum(dim=1).clamp(min=1e-8)
        corr_probs = torch.exp(weighted_log_sum / weight_sum)
    elif agg == "mean-geo":
        log_probs = torch.log(kc_probs.clamp(min=1e-8))
        weighted_log_sum = (log_probs * attention_weights * mask).sum(dim=1)
        n_kcs = mask.sum(dim=1).clamp(min=1)
        corr_probs = torch.exp(weighted_log_sum / n_kcs)
    else:
...
def get_kc_attention_weights(
    ...
    Uses the attention from deeper layers, focusing on the last KC position's
    attention to the context, as a proxy for how well the model is using evidence.

    Args:
        outputs: Model outputs with attentions
        batch: Batch dict with last_idxs and num_kcs
        layer_span: Evidence-reading layer range

    Returns:
        weights: (batch, max_kcs) attention-based weight per KC
    """
    ...
    for b in range(batch_size):
        for k in range(max_kcs):
            ...
            # Average attention from this KC position to all earlier tokens
            # across evidence-reading layers
            kc_att = 0.0
            for l in range(layer_range[0], layer_range[1]):
                att = attentions[l][0]  # (n_heads, seq, seq)
                # Mean across heads
                att_mean = att.mean(dim=0)
                # Attention from kc_pos to all earlier positions
                if kc_pos < att_mean.shape[0]:
                    kc_att += att_mean[kc_pos, :kc_pos + 1].mean().item()

            n_layers_used = max(1, layer_range[1] - layer_range[0])
            weights[b, k] = kc_att / n_layers_used

    # Normalize weights per sample (avoid division by zero)
    weight_sums = weights.sum(dim=1, keepdim=True).clamp(min=1e-8)
    weights = weights / weight_sums * batch["num_kcs"].unsqueeze(1).float().clamp(min=1)
